...
const KoreaMap = ({ localSearch, setLocalSearch }) => {
  const svgRef = useRef(null);
  const navigate = useNavigate(); // useNavigate 훅 사용
  const handleRegionChange = (newLocal) => {
    console.log('Region changed to:', newLocal);
    setLocalSearch(newLocal);
  };
  useEffect(() => {
    ...
    svg
      ...
      .attr('fill', (d) => {
        ...
        return localSearch === d.properties?.CTP_ENG_NM ? colorMapping[engName] : '#DFDFDF';
      })
      ...
      .on('click', function (event, d) {
        const engName = d.properties?.CTP_ENG_NM;
        if (engName) {
          const newLocal = localSearch === engName ? 'ALL' : engName;
          handleRegionChange(newLocal); // Directly pass the intended state

          // Update the map colors after region change
          svg.selectAll('path').attr('fill', (d) => {
            const engName = d.properties?.CTP_ENG_NM;
            if (localSearch === engName) {
              return '#DFDFDF'; // If deselecting, turn all to gray
            }
            return engName === d.properties?.CTP_ENG_NM ? colorMapping[engName] : '#DFDFDF';
          });
        }
      });
